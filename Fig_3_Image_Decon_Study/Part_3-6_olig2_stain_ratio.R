#Analysis of computed features from olig2 Stain Ratio
library(ggpubr)
...
ratio_frame <- read.csv("olig2_ratio_frame.csv")
...
ggplot(df_ratio, aes(x = Subtype, y = Ratio, color = Subtype)) +
  ...
  theme_classic() + theme(legend.position = "none", axis.text.x = element_text(angle = 45, hjust=1))  + ylab("Olig2 D:H Ratio") +
