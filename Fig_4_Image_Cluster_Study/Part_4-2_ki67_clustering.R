...
#Read in the table of segmentations (from the distance analysis)
seg_reads <- read.csv("ki67_Segmentation_masterframe.csv")
nrow(seg_reads) #97222
...
cell.table <- subset(cell.table, select = -c(X, Intensity, Case, Image)) #Remove miscellaneous
...
#Filter the base table so it aligns for image pull
...
umap_df <- readRDS("ki67_umap_df.RDS")
cell.table <- readRDS("ki67_sampled_cells.RDS")
...
filtered_umap_df %>%
  ...
  geom_col() +
  geom_text(position=position_stack(0.5), size = 3, color = "white", angle = 90)+
  theme_classic()+
