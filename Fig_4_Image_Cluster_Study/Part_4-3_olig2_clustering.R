...
seg_reads <- readRDS("olig2_Segmentation_masterframe.RDS")
nrow(seg_reads) #39877
...
base.table <- base.table[umap_df$UMAP1 > -25 & umap_df$UMAP1 < 0,]
nrow(base.table)
umap_df1 <- umap_df[umap_df$UMAP1 > -25 & umap_df$UMAP1 < 0,]
nrow(umap_df1)

base.table <- base.table[umap_df1$UMAP2 > 15 & umap_df1$UMAP2 < 27,]
nrow(base.table)
umap_df2 <- umap_df1[umap_df1$UMAP2 > 15 & umap_df1$UMAP2 < 27,]
nrow(umap_df2)
...
umap_df <- readRDS("olig2_umap_df.RDS")
cell.table <- readRDS("olig2_sampled_cells.RDS")
...
umap_df %>%
  ...
  labs(x = "UMAP1",
       y = "UMAP2") +
  theme_classic()+
  scale_color_manual(values = c("grey", brewer.pal(n = 5, name = "Dark2")))+
  theme(legend.position = "none")
