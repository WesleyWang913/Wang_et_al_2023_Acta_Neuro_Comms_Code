...
##############Please note UMAP is computationally heavy and a finalized version of processed data is present in the reposittory below#################
#Read in the table of segmentation (from the distance analysis)
seg_reads <- readRDS("he_Segmentation_masterframe.RDS")
nrow(seg_reads) #194378
#Split off PD Cases
...
cell.table <- subset(cell.table, select = -c(Case, Image)) #Remove miscellaneous
#####Note I found the color layers are just redundant, so I'm just gonna take the red layers
...
umap_df <- readRDS("he_umap_df.RDS")
cell.table <- readRDS("he_sampled_cells.RDS")

#Add Cluster Data (Note: make 0 cluster grey and rename NA)
umap_df %>%
  ggplot(aes(x = UMAP1, 
             y = UMAP2,
             color = Cluster))+
  ...
  labs(x = "UMAP1",
       y = "UMAP2") +
  theme_classic()+
  scale_color_manual(values = c("grey", brewer.pal(n = 3, name = "Dark2")))+
  theme(legend.position = "none")
...
filtered_umap_df %>%
  ...
  geom_col() +
  geom_text(position=position_stack(0.5), size = 4, color = "white", angle = 90)+
  theme_classic()+
...
ttable <- cbind(cell.table, umap_df$Cluster)
