...
seg_reads <- readRDS("p53_Segmentation_masterframe.RDS")
nrow(seg_reads) #171335
...
nrow(psPD.table) #153294
psPD.table$Status <- "psPD"

#Sample data to remove class inbalance (8,000 per side)
set.seed(888)
PD.sample <- PD.table[sample(nrow(PD.table), 8000),]
psPD.sample <- psPD.table[sample(nrow(psPD.table), 8000),]
...
cell.table <- subset(cell.table, select = -c(Intensity, Case, Image)) #Remove miscellaneous
...
base.table <- base.table[umap_df$UMAP1 > -33 & umap_df$UMAP1 < -15,]
nrow(base.table)
umap_df1 <- umap_df[umap_df$UMAP1 > -33 & umap_df$UMAP1 < -15,]
nrow(umap_df1)

base.table <- base.table[umap_df1$UMAP2 > -15 & umap_df1$UMAP2 < 10,]
nrow(base.table)
umap_df2 <- umap_df1[umap_df1$UMAP2 > -15 & umap_df1$UMAP2 < 10,]
nrow(umap_df2)

umap_df <- umap_df2

umap_df <- umap_df[umap_df$UMAP1 > -33 & umap_df$UMAP1 < -15,]
umap_df <- umap_df[umap_df$UMAP2 > -15 & umap_df$UMAP2 < 10,]
...
set.seed(123)
db <- fpc::dbscan(umap_df[,1:2], eps = 1, MinPts = 200)
umap_df$Cluster <- as.factor(db$cluster)

##########################Start Here For Reanalysis####################
umap_df <- readRDS("p53_umap_df.RDS")
cell.table <- readRDS("p53_sampled_cells.RDS")
...
umap_df %>%
  ...
  labs(x = "UMAP1",
       y = "UMAP2") +
  theme_classic()+
  scale_color_manual(values = c("grey", brewer.pal(n = 7, name = "Dark2")))+
  theme(legend.position = "none")
...
filtered_umap_df %>%
  ...
  geom_col() +
  geom_text(position=position_stack(0.5), size = 3.3, color = "white", angle = 90)+
  theme_classic()+
